...
class TestTqChecker(unittest.TestCase):

    resources_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'resources')

    def setUp(self):
        """Runs before each test."""
        self.preconvert_dir = os.path.join(self.resources_dir, 'some-tq-dir')  # Change when we have something to test
        self.temp_dir = tempfile.mkdtemp(prefix='temp_tq_')
        self.converted_dir = os.path.join(self.temp_dir, 'tq')
    ...
    def test_success(self):
        ...
        checker = TqChecker(self.preconvert_dir, self.converted_dir)
        checker.run()
